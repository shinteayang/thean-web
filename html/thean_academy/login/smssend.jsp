...
<%@ page import="DBPKG.Prepare"%>
    ...
    if(action.equals("go")) {
        ...
        String user_id = base64Encode("shinteayang"); // SMS아이디
        String secure = base64Encode("88a4aaf155b358516d5ebd13bf5e48d3");//인증키
    ...
    String sqla="";
        ...
        try {
            ...
            String key = Key.getKey();
     		sqla="select * from USER where user_phone=hex(AES_ENCRYPT("+request.getParameter("rphone")+", '" + key + "'))";
          
            PreparedStatement pstmt = conn.prepareStatement(sqla);
     	 
             rtes = pstmt.executeQuery();
         
            
				if(rtes.next()==false){
					%>
					<script>
					alert("등록되어있지 않음 사용자 정보입니다!");
					</script>
					 <script>
			           history.go(-1) 
			           </script>
